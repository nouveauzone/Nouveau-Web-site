...
const STATUS_COLORS = { pending:{bg:"#fff3cd",text:"#856404"}, processing:{bg:"#cce5ff",text:"#004085"}, shipped:{bg:"#d4edda",text:"#155724"}, delivered:{bg:"#d1ecf1",text:"#0c5460"}, cancelled:{bg:"#f8d7da",text:"#721c24"} };
...
export default function AdminPage({ setPage }) {
  ...
  const filteredOrders = allOrders.filter(o => {
    ...
    const matchQ = !q || (o._id||"").toLowerCase().includes(q) || (o.customer||o.user?.name||"").toLowerCase().includes(q) || (o.city||o.shippingAddress?.city||"").toLowerCase().includes(q);
    return matchQ && (orderFilter==="all" || (o.status||o.orderStatus)===orderFilter);
  });
  ...
  const handleUpdateOrderStatus = async (orderId, status) => {
    try { await API.updateOrderStatus(orderId, status); } catch {}
    setOrders(prev=>prev.map(o=>o._id===orderId?{...o,orderStatus:status,status}:o));
    if (ctxUpdateStatus) ctxUpdateStatus(orderId, status); // update context too
    if (selectedOrder?._id===orderId) setSelectedOrder(o=>({...o,orderStatus:status,status}));
  ...
  const getOrderStatus = (o) => o.status || o.orderStatus || "pending";
